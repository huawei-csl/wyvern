...
def format_prompt(prompt_template: str, 
    ...
    - report_a [`str`]: report A to be evaluated
    - report_a [`str`]: report B to be evaluated
...
def get_scores_dict(reports: tuple[str, str],
                    topic: str,
                    model: ChatOpenAI,
                    prompt_template_path: str,
                    rubric_path: str) -> dict:
    """Get the result of the pairwise relative evaluation with the model.
    
    Parameters
    ----------
    - reports [`tuple[str, str]`]: text of the reports to be evaluated
    - topic [`str`]: topic of the report
    ...
    for rubric in rubrics:
        ...
        prompt = format_prompt(prompt_template=prompt_template, 
                               topic=topic, 
                               rubric=rubric, 
                               report_a=reports[0],
                               report_b=reports[1])
...
def main(args):
    ...
    if args.save_path is None:
        SAVE_PATH = (Path(args.report_A_path).parents[1] / "evaluation" / "automatic" / "relative").as_posix()
    else:
    ...
    logger.info(f"Evaluating reports:\n\tA: '{args.report_A_path}'\n\tB: '{args.report_B_path}'")
    logger.info(f"Evaluating reports with {args.model_id}")

    evaluation_results = {}
    evaluation_results["report_A_path"] = args.report_A_path
    evaluation_results["report_B_path"] = args.report_B_path
    ...
    # Report A
    with open(args.report_A_path) as f:
        report_A = f.read()
    fig_masker = FiguresMasker()
    report_A = fig_masker.replace_base64_with_links(report_A)
    report_A = report_A.replace("<sup>", "").replace("</sup>", "").replace("<small>", "").replace("</small>", "")
    report_A = remove_urls(report_A)
    if "## References" in report_A:
        report_A = report_A.split("## References")[0]
    else:
        report_A = report_A.split("# References")[0]

    # Report B
    with open(args.report_B_path) as f:
        report_B = f.read()
    fig_masker = FiguresMasker()
    report_B = fig_masker.replace_base64_with_links(report_B)
    report_B = report_B.replace("<sup>", "").replace("</sup>", "").replace("<small>", "").replace("</small>", "")
    report_B = remove_urls(report_B)        
    if "## References" in report_B:
        report_B = report_B.split("## References")[0]
    else:
        report_B = report_B.split("# References")[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--report-A-path", type=str, required=True,
                        help="Path of one of the reports to be evaluated.")
    parser.add_argument("--report-B-path", type=str, required=True,
                        help="Path of one of the reports to be evaluated.")
    ...
    parser.add_argument("--save-path", type=str, default=None,
                        ...
                        the results will be saved in the evaluation folder of report A.""")
    parser.add_argument("--prompt-template-path", type=str,
                        default=str(Path(__file__).resolve().parent / "resources" / "template_relative_eval.prompt"),
